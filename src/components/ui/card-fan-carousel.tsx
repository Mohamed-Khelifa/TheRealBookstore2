...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border border-white/20 bg-white/20 backdrop-blur-md text-white cursor-pointer shrink-0 z-30 outline-none shadow-xl hover:bg-white/30 hover:border-white/40 active:opacity-70 transition-all duration-300";
          ...
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl shadow-xl border border-white/10 bg-black/20 dark:bg-white/5 backdrop-blur-sm">
                <LazyImage src={card.imgUrl} alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-contain z-10" />
              </div>
            );
            return card.linkUrl ? (
